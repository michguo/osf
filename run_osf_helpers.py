...
def normalize_raw(raw, z_vals, rays_d, raw_noise_std, scaled_sigmoid, density_scale):
    ...
    if density_scale > 1:
        breakpoint()
    alpha = compute_alpha(
        z_vals=z_vals, raw_alpha=raw[..., 3] * density_scale, rays_d=rays_d, raw_noise_std=raw_noise_std)
    return rgb, alpha
...
def run_single_object(
    ...
    rgb, alpha = normalize_raw(raw, z_vals, rays_d, raw_noise_std, scaled_sigmoid, density_scale)
    ret = {
        'z_vals': z_vals,
        'rgb': rgb,
        'alpha': alpha,
...
def combine_multi_object_results(name2results, object_params):
    ...
    results = {}
    for k in ['z_vals', 'rgb', 'alpha', 'raw', 'ray_batch', 'shadow_ray_batch', 'pts']:
        if k == 'z_vals':
            v_combined = z_vals
        else:
...
def select_rays_by_mask(rays_rgb_all, rays_mask, outmask_keep_ratio):
    """
    args:
        rays_rgb_all: [N, H, W, :, :]
        rays_mask: [N, H, W]
        outmask_keep_ratio: ranging from 0 to 1
    return:
        rays_rgb: [N_selected, :, :]
    """

    inmask_ray = rays_rgb_all[rays_mask > 0.01]  # remove black pixels
    rays_mask = rays_mask.reshape(-1)
    outmask_idx = np.where(rays_mask <= 0.01)[0]
    n_outmask_to_keep = int(outmask_idx.shape[0] * outmask_keep_ratio)
    kept_outmask_idx = np.random.choice(outmask_idx, size=n_outmask_to_keep, replace=False)
    rays_rgb_all = np.reshape(rays_rgb_all, [-1] + list(rays_rgb_all.shape[-2:]))  # [N*H*W, ro+rd+ri+rgb, 3]
    kept_outmask_ray = rays_rgb_all[kept_outmask_idx]

    rays_rgb = np.concatenate([inmask_ray, kept_outmask_ray], axis=0)
